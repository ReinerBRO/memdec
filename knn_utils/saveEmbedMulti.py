...
def normalize_key_type(knn_keytype):
    if knn_keytype is None:
        return KEY_TYPE.last_ffn_input
    if isinstance(knn_keytype, KEY_TYPE):
        return knn_keytype
    if isinstance(knn_keytype, str):
        return KEY_TYPE.from_string(knn_keytype)

    name = getattr(knn_keytype, "name", None)
    if name is not None:
        return KEY_TYPE.from_string(name)

    value = getattr(knn_keytype, "value", None)
    if isinstance(value, str):
        return KEY_TYPE.from_string(value)

    raise ValueError(f"Unsupported knn_keytype: {knn_keytype!r}")
...
class KNNWrapperMulti(object):
    def __init__(self, val_file, index_file, dimension, 
        ...
        self.knn_keytype = normalize_key_type(knn_keytype)
